...
bool AnimationLoader::loadAnimation(const std::wstring& bitmapFile, const std::wstring& jsonFile, _Animation& root, float duration) {
	...
	Src_Bitmap = Resource::Instance().GetBitmap(bitmapFile); //加载资源图片
	...
	for (int j = 0; j < animation_num; j++)
	{
		...
		for (int i = from; i != to + 1; i++)
		{
			...
			root[name].first.at(count) = CreateSpritesBitmap(Src_Bitmap, &anim_data);//按照配置文件裁剪图片
...
ID2D1Bitmap1* AnimationLoader::CreateSpritesBitmap(ID2D1Bitmap1* srcBitmap, animation_data* d)
{
	...
	m_content->SetTransform(D2D1::Matrix3x2F::Translation(0.0f, 0.0f));	// 记得先归位摄像机
	m_content->BeginDraw();
...
{
	auto anim =AnimationData.find(std::make_pair(bitmapFile, jsonFile));//查找文件是否被加载过
	...
	if (anim ==AnimationData.end())
	{
		...
		if (!loadAnimation(bitmapFile, jsonFile, AnimationData.at(std::make_pair(bitmapFile, jsonFile)), duration))//加载动画文件
			return nullptr;
		else
